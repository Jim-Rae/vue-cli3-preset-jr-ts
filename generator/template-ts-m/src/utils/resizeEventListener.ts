...
const cancelFrame = (function () {
  const cancel = window.cancelAnimationFrame || window.webkitCancelAnimationFrame || window.clearTimeout
 ...
 */
const resetTrigger = function (element: any) {
 ...
 * @param event 要监听的事件
 */
const scrollListener = function (this: any, event: any) {
  resetTrigger(this) // 重置触发器
  if (this.__resizeRAF__) !isServer && cancelFrame(this.__resizeRAF__) // 取消之前的回调
  this.__resizeRAF__ = !isServer && requestFrame(() => { // 设置新的函数
    if (checkTriggers(this)) { // 如果改变了大小
      ...
      this.__resizeListeners__.forEach((fn: any) => { // 调用回调
        fn.call(this, event)
      })
    }
  })
 ...
 */
const attachEvent = isServer ? {} : (document as any).attachEvent
...
if (!attachEvent && !isServer) {
  ...
  if (animation === false) {
    ...
    for (var i = 0; i < DOM_PREFIXES.length; i++) {
      if ((testElement.style as any)[DOM_PREFIXES[i] + 'AnimationName'] !== undefined) { // 根据testElement来检测支持的前缀
  ...
  if (attachEvent) { // 处理低版本ie
    ...
    if (!element.__resizeTrigger__) { // 如果没有触发器
      ...
      if (animationStartEvent) { // 动画开始
        resizeTrigger.addEventListener(animationStartEvent, function (event: any) { // 增加动画开始的事件监听
